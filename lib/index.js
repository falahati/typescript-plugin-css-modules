...
function init(_a) {
    ...
    function create(info) {
        ...
        ts.createLanguageServiceSourceFile = function (fileName, scriptSnapshot) {
            ...
            if (isCSS(fileName)) {
                scriptSnapshot = getDtsSnapshot_1.getDtsSnapshot(ts, processor, fileName, scriptSnapshot, options, logger, compilerOptions, directory);
            }
        ...
        ts.updateLanguageServiceSourceFile = function (sourceFile, scriptSnapshot) {
            ...
            if (isCSS(sourceFile.fileName)) {
                scriptSnapshot = getDtsSnapshot_1.getDtsSnapshot(ts, processor, sourceFile.fileName, scriptSnapshot, options, logger, compilerOptions, directory);
            }
